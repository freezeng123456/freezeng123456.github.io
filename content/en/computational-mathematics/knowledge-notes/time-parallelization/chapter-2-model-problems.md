...
Section 2 contains no algorithms. It settles a more basic question, namely which evolution problems are suited to time parallelization at all, and Chapters 3 and 4 draw their grouping of methods entirely from it.

Causality tells us that the late solution is determined by the early one, which makes the time direction appear inherently sequential, yet the paper opens by asking whether we can compute $t\in(1.7,2.2)$ before knowing the solution for $t<1.7$. For the heat equation with Dirichlet boundaries the answer is yes, and the paper marks it with an exclamation point.

Causality describes a logical dependence, whereas what matters in computation is the numerically effective dependence. Once dissipation has erased old information, its influence on the late solution falls below the discretization error, and the requirement to know the early solution first no longer holds in any computational sense. The rest of the chapter measures one thing throughout, which is how long the solution's memory lasts and which frequencies that memory keeps. Those two answers decide directly whether time parallelization can work.

## The four figures form a single controlled experiment

The four figures form one controlled experiment. The paper reuses the same source term and the same oscillatory initial condition across all four equations and varies only three things: the equation itself, moving from the heat equation through advection–diffusion and Burgers to the wave equation and adding transport and nonlinearity along the way; the boundary condition, switching among Dirichlet, Neumann, and periodic to decide whether information can leave the domain; and the diffusion strength $\nu$, taking $1$, $10^{-2}$, and $5\times10^{-4}$ to decide how fast fine scales decay. Everything else is held fixed, so the panels can be compared directly against one another.

The last column of each figure switches to a different experiment. Figures 2.1(d), 2.2(d) and (h), 2.3(d) and (h), and 2.4(d) all turn the source off and use the oscillatory initial condition. The earlier columns ask how long the response to an external forcing survives, the last column how long the initial information itself survives.

Most time-parallel methods are described and analyzed on the system of ordinary differential equations obtained after spatial semidiscretization. The linear case is written
...
All four models are posed on the one-dimensional unit interval $\Omega=(0,1)$. The applicability and convergence properties of time-parallel methods generally do not depend on the spatial dimension, so a one-dimensional setting costs nothing here. Dimension changes the cost of a single spatial solve considerably; the three mechanisms this chapter cares about are unaffected: how old information decays, whether fine scales survive, and how information leaves or returns through a boundary.
...
Only a very thin band of stripes survives near $t\approx0$, and above it the panel becomes an almost uniform block of colour. That block is the one component that survives after the periodic heat equation rapidly damps every nonconstant Fourier mode, namely the spatial mean of the initial condition. The paper adds a quantitative comparison: this surviving constant is roughly the same as the constant accumulated from the first two sources in panels (b) and (c).

The Dirichlet case is highly local in time, as analyzed in Gander, Ohlberger and Rave (2024), and this temporal locality can be compared with the spatial locality of solvation models in computational chemistry, see Ciaramella and Gander (2017, 2018a, 2018b). The Neumann and periodic cases still leave room for time parallelization, provided the algorithm can carry low-frequency components such as the constant mode accurately over long times, and a coarse grid is the mechanism the paper points to. Put differently, high-frequency error disappears on its own through dissipation and needs no communication, while low-frequency components never decay by themselves and must be transported across time. Everything Parareal, PFASST, MGRiT, and STMG do in Chapter 4 rests on that premise.
...
with $c>0$, and the figure uses $c^2=0.2$.
...
The wave equation exposes the difficulty without periodic boundaries because of the directionality of propagation. The transport term in advection–diffusion and Burgers is first order and information travels one way, so a Dirichlet boundary becomes an exit and periodicity has to close that exit before the difficulty shows. The wave equation propagates in several directions, and Dirichlet and Neumann boundaries reflect the waves back, so the domain has no exit at all. The equation also has no dissipative term, no mode decays, and phase error simply accumulates. Once a coarse temporal model is off in phase or wave speed, the dynamics will not correct it, and an effective method has to transport fine detail over long times itself. Chapter 3 turns to SWR, parallel IDC, ParaExp, and ParaDiag precisely to address this.
...
Three criteria carry over directly to the test configuration of every algorithm that follows: does the boundary condition let information leave, how fast does the diffusion parameter make fine scales decay, and will nonlinearity regenerate high frequencies.

The following summary is this site's own framing. The differences among the four models come down to the relationship between the coarse and fine representations: a coarse propagator is essentially a low-pass filter, and parabolic dynamics is one as well, discarding high frequencies of its own accord, so the coarse solver and the true dynamics discard the same information and agree on what matters. Hyperbolic dynamics has no such property and carries every frequency unchanged far into the future, so the coarse solver discards exactly the information that determines the answer. That is why the methods of Chapter 4 fail on the problems of Chapter 3, and why the paper abandons a classification by algorithmic lineage in favour of one by dynamics.
